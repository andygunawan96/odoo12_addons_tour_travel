...
class TtCronLogInhAccPassport(models.Model):
    ...
    def cron_send_passport_transactions_to_vendor(self):
        ...
        for ho_obj in ho_objs:
            try:
                self.env['tt.reservation.passport'].send_transaction_batches_to_accounting(1, ho_obj.id)
            except:
                self.create_cron_log_folder()
                self.write_cron_log('auto-send passport transactions to vendor', ho_id=ho_obj.id)
