...
class TransportSegment(models.Model,test_to_dict.ToDict):
    ...
    carrier_id  = fields.Many2one('tt.transport.carrier','Train')
    carrier_name = fields.Char('Train Name')
    carrier_code = fields.Char('Train Code')
    carrier_number = fields.Char('Train Number')
    ...
    journey_id = fields.Many2one('tt.tb.journey.train', 'Journey', ondelete='cascade')
    booking_id = fields.Many2one('tt.reservation.train', 'Order Number', related='journey_id.booking_id', store=True)

    # additional_service_charge_ids = fields.One2many('tt.tb.service.charge', 'segment_id')
    seat_ids = fields.One2many('tt.tb.seat.train', 'segment_id', 'Seat')
    leg_ids = fields.One2many('tt.tb.leg.train', 'segment_id', 'Legs')
    ...
    def _compute_date_fmt(self):
        ...
        for rec in self:
            rec.departure_date_fmt = rec.departure_date and datetime.strptime(rec.departure_date, '%Y-%m-%d %H:%M:%S').strftime("%s %s" % (lang.date_format, lang.time_format)) or ''
            rec.arrival_date_fmt = rec.arrival_date and datetime.strptime(rec.arrival_date, '%Y-%m-%d %H:%M:%S').strftime("%s %s" % (lang.date_format, lang.time_format)) or ''
