...
class TtProviderphc(models.Model):
    _name = 'tt.provider.phc'
    _inherit = 'tt.history'
    _rec_name = 'pnr'
    # _order = 'departure_date'
    _description = 'Provider phc'

    pnr = fields.Char('PNR')
    pnr2 = fields.Char('PNR2')
    provider_id = fields.Many2one('tt.provider','Provider')
    state = fields.Selection(variables.BOOKING_STATE, 'Status', default='draft')
    booking_id = fields.Many2one('tt.reservation.phc', 'Order Number', ondelete='cascade')
    ...
    ticket_ids = fields.One2many('tt.ticket.phc', 'provider_id', 'Ticket Number')
    ...
    sid_cancel = fields.Char('SID Cancel', readonly=True,
                             states={'draft': [('readonly', False)]})  # signature generate sendiri
    cost_service_charge_ids = fields.One2many('tt.service.charge', 'provider_phc_booking_id', 'Cost Service Charges')
    ...
    error_history_ids = fields.One2many('tt.reservation.err.history', 'res_id', 'Error History',
                                        domain=[('res_model', '=', 'tt.provider.phc')])
    ...
    def action_force_issued_from_button(self, payment_data={}):
        ...
        payment_res = self.booking_id.payment_reservation_api('phc', req, context)
    ...
    def action_reverse_ledger_from_button(self):
        if not self.env.user.has_group('tt_base.group_reservation_provider_level_4'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 248')
    ...
    def action_set_to_book_from_button(self):
        if not self.env.user.has_group('tt_base.group_reservation_provider_level_4'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 249')
    ...
    # def action_booked_api_phc(self, provider_data, api_context):
    #     for rec in self:
    #         rec.write({
    ...
    def action_issued_phc(self, co_uid):
        for rec in self:
            rec.write({
                'state': 'issued',
                'issued_date': datetime.now(),
    ...
    def action_cancel_api_phc(self, context):
        for rec in self:
            rec.write({
                'state': 'cancel',
    ...
    def action_cancel_pending_api_phc(self, context):
        for rec in self:
            rec.write({
    ...
    def action_failed_booked_api_phc(self, err_code, err_msg):
        for rec in self:
            rec.write({
                'state': 'fail_booked',
    ...
    def action_failed_void_api_phc(self, err_code, err_msg):
        for rec in self:
            rec.write({
                'state': 'void_failed',
                'error_history_ids': [(0, 0, {
    ...
    def action_cancel(self, context=False):
        ...
        self.state = 'cancel'
        self.env['tt.phc.api.con'].send_cancel_order_notification(self.booking_id.name,
                                                                        self.env.user.name,
    ...
    def create_service_charge(self, service_charge_vals):
        ...
        for scs in service_charge_vals:
            ...
            scs['provider_phc_booking_id'] = self.id
            for psg in self.ticket_ids:
                if scs['pax_type'] == psg.pax_type and scs_pax_count < scs['pax_count']:
                    scs['passenger_phc_ids'].append(psg.passenger_id.id)
                    # scs['pax_count'] += 1
                    scs_pax_count += 1
                    scs['total'] += scs['amount']
            scs['passenger_phc_ids'] = [(6, 0, scs['passenger_phc_ids'])]
    ...
    def get_carriers_api(self):
        data = self.env['tt.transport.carrier'].search([('provider_type_id','=',self.env.ref('tt_reservation_phc.tt_provider_type_phc').id)])
        res = []
        for rec in data:
            res.append({
                "name": rec['name'],
    ...
    def update_ticket_number_pax(self):
        if not self.env.user.has_group('base.group_system'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 250')
