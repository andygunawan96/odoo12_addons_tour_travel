...
class AgentReportOfflineXls(models.TransientModel):
    _inherit = 'tt.agent.report.ho.offline.wizard'
    ...
    def _print_report_excel(self, data):
        ...
        values = self.env['report.tt_agent_report_offline.agent_report_offline_ho']._prepare_values(data['form'])  # get values
        ...
        sheet.set_column('F:H', 9)
        sheet.set_column('I:I', 18)
        sheet.set_column('J:M', 6)
        ...
        for rec in values['lines']:
            ...
            sheet.write(row_data, 14, rec['agent_commission'], sty_amount)
            sheet.write(row_data, 15, rec['parent_commission'], sty_amount)
            sheet.write(row_data, 16, rec['ho_commission'], sty_amount)
            sheet.write(row_data, 17, rec['total_commission'], sty_amount)
            sheet.write(row_data, 18, rec['nta_amount'], sty_amount)
            sheet.write(row_data, 19, rec['state'], sty_table_data_center)
            sheet.set_row(row_data, row_height)

        workbook.close()

        attach_id = self.env['tt.agent.report.excel.output.wizard'].create(
            {'name': 'Reservation Offline Report HO.xlsx', 'file_output': base64.encodebytes(stream.getvalue())})
        return {
