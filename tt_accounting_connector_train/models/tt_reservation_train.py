...
class TtReservationTrain(models.Model):
    ...
    def action_issued_train(self,data):
        ...
        setup_list = self.env['tt.accounting.setup'].search(
            [('cycle', '=', 'real_time'), ('is_send_train', '=', True)])
        if setup_list:
            vendor_list = []
    ...
    def action_reverse_train(self,context):
        ...
        if old_state == 'issued':
            temp_post = self.posted_acc_actions or ''
            setup_list = self.env['tt.accounting.setup'].search(
                [('cycle', '=', 'real_time'), ('is_send_train', '=', True), ('is_send_reverse_transaction', '=', True)])
            if setup_list:
                vendor_list = []
                for rec in setup_list:
                    if rec.accounting_provider not in vendor_list:
    ...
    def send_transaction_batches_to_accounting(self, days):
        ...
        for rec in transaction_list:
            ...
            if 'reconcile' not in temp_post.split(',') and 'transaction_batch' not in temp_post.split(','):
                setup_list = self.env['tt.accounting.setup'].search(
                    [('cycle', '=', 'per_batch'), ('is_recon_only', '=', False), ('is_send_train', '=', True)])
                if setup_list:
                    vendor_list = []
