...
class TtCronLogInhAccTrain(models.Model):
    ...
    def cron_send_train_transactions_to_vendor(self):
        ...
        for ho_obj in ho_objs:
            try:
                self.env['tt.reservation.train'].send_transaction_batches_to_accounting(1, ho_obj.id)
            except:
                self.create_cron_log_folder()
                self.write_cron_log('auto-send train transactions to vendor', ho_id=ho_obj.id)
