...
class TtReservationOffline(models.Model):
    ...
    def action_cancel(self):
        ...
        if old_state == 'issued':
            temp_post = self.posted_acc_actions or ''
            setup_list = self.env['tt.accounting.setup'].search(
                [('cycle', '=', 'real_time'), ('is_send_offline', '=', True), ('is_send_reverse_transaction', '=', True)])
            if setup_list:
                vendor_list = []
                for rec in setup_list:
                    if rec.accounting_provider not in vendor_list:
    ...
    def send_transaction_batches_to_accounting(self, days):
        ...
        transaction_list = self.env['tt.reservation.offline'].search([('state', '=', 'issued'), ('issued_date', '>=', start_datetime)])
