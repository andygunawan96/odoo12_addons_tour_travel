...
class ReservationPeriksain(models.Model):
    ...
    def action_issued_api_periksain(self,req,context):
        data = {
        ...
        self.action_issued_periksain(data)
    ...
    def check_provider_state(self,context,pnr_list=[],hold_date=False,req={}):
        ...
        if all(rec.state == 'issued' for rec in self.provider_booking_ids):
            ...
            self.action_issued_api_periksain(issued_req, context)
    ...
    def print_ho_invoice(self):
        ...
        res = res and res[0] or {}
        datas['form'] = res
        periksain_ho_invoice_id = self.env.ref('tt_report_common.action_report_printout_invoice_ho_periksain')
    ...
    def print_itinerary_price(self, data, ctx=None):
        ...
        if not book_obj.printout_itinerary_price_id or data.get('is_force_get_new_printout', False):
            ...
            res = book_obj.env['tt.upload.center.wizard'].upload_file_api(
                {
                    'filename': 'Periksain Itinerary %s (Price).pdf' % book_obj.name,
                    'file_reference': 'Itinerary',
                    'file': base64.b64encode(pdf_report_bytes[0]),
                    'delete_date': datetime.today() + timedelta(minutes=10)
                },
    ...
    def get_terms_conditions_email_old(self):
        ...
        if self.carrier_name in ['PRKPCR']:
            ...
            terms_txt += "7. Under normal circumstances, test results will be sent via Email by Periksain.id around 2-4 hours after the test.<br/>"
    ...
    def get_passenger_pricing_breakdown(self):
        ...
        for rec in self.passenger_ids:
            ...
            for rec2 in self.provider_booking_ids:
                ...
                for rec3 in rec2.cost_service_charge_ids.filtered(lambda y: rec.id in y.passenger_periksain_ids.ids):
