...
class ReservationTrain(models.Model):
    ...
    def action_create_invoice(self):
        ...
        #untuk mark up 7500 admmin fee
        for svrc in self.sale_service_charge_ids:
            if not svrc.charge_code in ['fare', 'r.ac', 'r.ac1', 'r.ac2']:
                inv_line_obj.write({
                    'invoice_line_detail_ids': [(0, 0, {
                        'desc': get_charge_code_name(svrc.charge_code),
                        'price_unit': svrc.amount,
                        'quantity': svrc.pax_count,
                        'invoice_line_id': invoice_line_id,
                    })]
                })
