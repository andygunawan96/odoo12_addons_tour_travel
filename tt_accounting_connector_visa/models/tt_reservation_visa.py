...
class TtReservationVisa(models.Model):
    ...
    def action_issued_visa_api(self, data, context):
        ...
        if self.agent_id.is_sync_to_acc:
            ...
            if 'issued' not in temp_post.split(','):
                ...
                search_params = [('cycle', '=', 'real_time'), ('is_send_visa', '=', True)]
                if ho_obj:
                    search_params.append(('ho_id', '=', ho_obj.id))
    ...
    def action_reverse_visa(self,context):
        ...
        if self.agent_id.is_sync_to_acc:
            if old_state == 'issued':
                temp_post = self.posted_acc_actions or ''
                ho_obj = self.agent_id and self.agent_id.ho_id or False
                search_params = [('cycle', '=', 'real_time'), ('is_send_visa', '=', True), ('is_send_reverse_transaction', '=', True)]
                if ho_obj:
                    search_params.append(('ho_id', '=', ho_obj.id))
                setup_list = self.env['tt.accounting.setup'].search(search_params)
                if setup_list:
    ...
    def send_transaction_batches_to_accounting(self, days, ho_id):
        ...
        transaction_list = self.env['tt.reservation.visa'].search([('state', '=', 'issued'), ('issued_date', '>=', start_datetime), ('ho_id','=',ho_id)])
        for rec in transaction_list:
            if rec.agent_id.is_sync_to_acc:
                ...
                if 'reconcile' not in temp_post.split(',') and 'transaction_batch' not in temp_post.split(','):
                    ...
                    search_params = [('cycle', '=', 'per_batch'), ('is_recon_only', '=', False), ('is_send_visa', '=', True)]
                    if ho_obj:
                        search_params.append(('ho_id', '=', ho_obj.id))
                    setup_list = self.env['tt.accounting.setup'].search(search_params)
                    if setup_list:
