...
class ConfirmOrderSwabExpressWizard(models.TransientModel):
    _name = "confirm.order.swabexpress.wizard"
    ...
    analyst_ids = fields.Many2many('tt.analyst.swabexpress', 'tt_reservation_swabexpress_analyst_confirm_order_wizard_rel', 'wizard_id',
    ...
    def confirm_order(self):
        if not ({self.env.ref('tt_base.group_external_vendor_swabexpress_level_2').id, self.env.ref('tt_base.group_reservation_level_4').id}.intersection(set(self.env.user.groups_id.ids))):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 291')
        ...
        try:
            self.env['tt.swabexpress.api.con'].send_confirm_order_notification(self.booking_id.name,
                                                                             self.env.user.name,
