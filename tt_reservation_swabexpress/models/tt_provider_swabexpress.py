...
class TtProviderSwabExpress(models.Model):
    _name = 'tt.provider.swabexpress'
    _inherit = 'tt.history'
    _rec_name = 'pnr'
    ...
    booking_id = fields.Many2one('tt.reservation.swabexpress', 'Order Number', ondelete='cascade')
    ...
    ticket_ids = fields.One2many('tt.ticket.swabexpress', 'provider_id', 'Ticket Number')
    ...
    sid_cancel = fields.Char('SID Cancel', readonly=True,
                             states={'draft': [('readonly', False)]})  # signature generate sendiri
    cost_service_charge_ids = fields.One2many('tt.service.charge', 'provider_swabexpress_booking_id', 'Cost Service Charges')
    ...
    def create(self, vals_list):
        rec = super(TtProviderSwabExpress, self).create(vals_list)
        rec.write({
    ...
    def action_force_issued_from_button(self, payment_data={}):
        ...
        payment_res = self.booking_id.payment_reservation_api('swabexpress', req, context)
    ...
    def action_reverse_ledger_from_button(self):
        if not self.env.user.has_group('tt_base.group_reservation_provider_level_4'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 282')
    ...
    def action_set_to_book_from_button(self):
        if not self.env.user.has_group('tt_base.group_reservation_provider_level_4'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 283')
    ...
    # def action_booked_api_swabexpress(self, provider_data, api_context):
    #     for rec in self:
    #         rec.write({
    ...
    def action_issued_api_swabexpress(self, context):
    ...
    def action_issued_swabexpress(self, co_uid):
        for rec in self:
            rec.write({
                'state': 'issued',
                'issued_date': datetime.now(),
    ...
    def action_cancel_api_swabexpress(self, context):
        for rec in self:
            rec.write({
                'state': 'cancel',
    ...
    def action_cancel_pending_api_swabexpress(self, context):
        for rec in self:
            rec.write({
    ...
    def action_failed_booked_api_swabexpress(self, err_code, err_msg):
        for rec in self:
            rec.write({
                'state': 'fail_booked',
    ...
    def action_failed_issued_api_swabexpress(self, err_code, err_msg):
        for rec in self:
            rec.write({
                'state': 'fail_issued',
                'error_history_ids': [(0, 0, {
    ...
    def action_failed_paid_api_swabexpress(self, err_code, err_msg):
        for rec in self:
            rec.write({
                'state': 'fail_paid',
                'error_history_ids': [(0, 0, {
    ...
    def action_failed_void_api_swabexpress(self, err_code, err_msg):
        for rec in self:
            rec.write({
                'state': 'void_failed',
                'error_history_ids': [(0, 0, {
    ...
    def action_cancel(self, context=False):
        ...
        self.state = 'cancel'
        self.env['tt.swabexpress.api.con'].send_cancel_order_notification(self.booking_id.name,
                                                                        self.env.user.name,
    ...
    def create_service_charge(self, service_charge_vals):
        ...
        for scs in service_charge_vals:
            ...
            scs['provider_swabexpress_booking_id'] = self.id
            for psg in self.ticket_ids:
                if scs['pax_type'] == psg.pax_type and scs_pax_count < scs['pax_count']:
                    scs['passenger_swabexpress_ids'].append(psg.passenger_id.id)
                    # scs['pax_count'] += 1
                    scs_pax_count += 1
                    scs['total'] += scs['amount']
            scs['passenger_swabexpress_ids'] = [(6, 0, scs['passenger_swabexpress_ids'])]
