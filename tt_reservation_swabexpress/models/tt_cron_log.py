...
class TtCronLogInhSwabExpress(models.Model):
    _inherit = 'tt.cron.log'

    # def cron_reverse_issued_pending(self):
    #     try:
    #         issued_pending_bookings = self.env['tt.reservation.swabexpress'].search(
    ...
    def cron_auto_done_state_vendor_swabexpress(self):
        try:
            issued_bookings = self.env['tt.reservation.swabexpress'].search(
                [('state','=','issued'),
    ...
    def cron_auto_create_timeslot_swabexpress(self):
        try:
            time_string = ''
            ...
            for rec in self.env['tt.destinations'].search(
                    [('provider_type_id','=',self.env.ref('tt_reservation_swabexpress.tt_provider_type_swabexpress').id)]):
