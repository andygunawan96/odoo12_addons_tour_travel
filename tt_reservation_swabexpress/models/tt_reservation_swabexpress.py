...
class ReservationSwabExpress(models.Model):
    _name = "tt.reservation.swabexpress"
    ...
    sale_service_charge_ids = fields.One2many('tt.service.charge', 'booking_swabexpress_id', 'Service Charge',
    ...
    provider_booking_ids = fields.One2many('tt.provider.swabexpress', 'booking_id', string='Provider Booking', readonly=True, states={'draft': [('readonly', False)]})

    timeslot_ids = fields.Many2many('tt.timeslot.swabexpress','tt_reservation_swabexpress_timeslot_rel', 'booking_id', 'timeslot_id', 'Timeslot(s)')

    picked_timeslot_id = fields.Many2one('tt.timeslot.swabexpress', 'Picked Timeslot', readonly=True, states={'draft': [('readonly', False)]})

    test_datetime = fields.Datetime('Test Datetime',related='picked_timeslot_id.datetimeslot', store=True)

    analyst_ids = fields.Many2many('tt.analyst.swabexpress', 'tt_reservation_swabexpress_analyst_rel', 'booking_id',
                                   'analyst_id', 'Analyst(s)', readonly=True, states={'draft': [('readonly', False)]})

    provider_type_id = fields.Many2one('tt.provider.type','Provider Type',
                                       default=lambda self: self.env.ref('tt_reservation_swabexpress.tt_provider_type_swabexpress'))

    split_from_resv_id = fields.Many2one('tt.reservation.swabexpress', 'Splitted From', readonly=1)
    split_to_resv_ids = fields.One2many('tt.reservation.swabexpress', 'split_from_resv_id', 'Splitted To', readonly=1)
    ...
    def action_set_as_draft(self):
        if not self.env.user.has_group('base.group_system'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 284')
        for rec in self:
            rec.state = 'draft'
    ...
    def action_set_as_booked(self):
        if not self.env.user.has_group('base.group_system'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 285')
        for rec in self:
            rec.state = 'booked'
    ...
    def action_set_as_issued(self):
        if not self.env.user.has_group('base.group_system'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 286')
        for rec in self:
            rec.state = 'issued'
    ...
    def action_set_state_vendor_as_test_completed(self):
        if not ({self.env.ref('tt_base.group_external_vendor_swabexpress_level_2').id, self.env.ref('tt_base.group_reservation_level_4').id}.intersection(set(self.env.user.groups_id.ids))):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 287')
        self.state_vendor = 'test_completed'

    def action_set_state_vendor_as_no_show(self):
        if not ({self.env.ref('tt_base.group_external_vendor_swabexpress_level_2').id, self.env.ref('tt_base.group_reservation_level_4').id}.intersection(set(self.env.user.groups_id.ids))):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 288')
        self.state_vendor = 'no_show'

    def action_set_state_vendor_as_refund(self):
        if not ({self.env.ref('tt_base.group_external_vendor_swabexpress_level_2').id, self.env.ref('tt_base.group_reservation_level_4').id}.intersection(set(self.env.user.groups_id.ids))):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 289')
    ...
    def action_booked_api_swabexpress(self,context):
        ...
        try:
            if self.agent_type_id.is_send_email_booked:
                mail_created = self.env['tt.email.queue'].sudo().with_context({'active_test':False}).search([('res_id', '=', self.id), ('res_model', '=', self._name), ('type', '=', 'booked_swabexpress')], limit=1)
                if not mail_created:
                    temp_data = {
                        'provider_type': 'swabexpress',
    ...
    def action_issued_swabexpress(self, data):
        ...
        try:
            if self.agent_type_id.is_send_email_issued:
                mail_created = self.env['tt.email.queue'].sudo().with_context({'active_test': False}).search([('res_id', '=', self.id), ('res_model', '=', self._name), ('type', '=', 'issued_swabexpress')], limit=1)
                if not mail_created:
                    temp_data = {
                        'provider_type': 'swabexpress',
                        'order_number': self.name,
    ...
    def action_reverse_swabexpress(self,context):
        self.write({
            'state':  'fail_refunded',
    ...
    def action_refund_failed_swabexpress(self,context):
        self.write({
            'state':  'refund_failed',
        })
    ...
    #
    #     try:
    #         mail_created = self.env['tt.email.queue'].sudo().with_context({'active_test':False}).search([('res_id', '=', self.id), ('res_model', '=', self._name), ('type', '=', 'issued_final_swabexpress')], limit=1)
    #         if not mail_created:
    #             temp_data = {
    #                 'provider_type': 'swabexpress',
    #                 'order_number': self.name,
    ...
    def action_cancel(self, backend_context=False, gateway_context=False):
        if not self.env.user.has_group('tt_base.group_reservation_level_4'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 290')
    ...
    def get_price_swabexpress_api(self, req, context):
        #parameter
        #timeslot_list
        #jumlah pax
        overtime_surcharge = False
        timeslot_objs = self.env['tt.timeslot.swabexpress'].search([('seq_id', 'in', req['timeslot_list'])])
        ...
        if req['pax_count'] <= 1 and \
                carrier_id == self.env.ref('tt_reservation_swabexpress.tt_transport_carrier_swabexpress_antigen').id:
    ...
    def update_pnr_provider_swabexpress_api(self, req, context):
        _logger.info("Update\n" + json.dumps(req))
        try:
            if req.get('book_id'):
                book_obj = self.env['tt.reservation.swabexpress'].browse(req['book_id'])
            elif req.get('order_number'):
                book_obj = self.env['tt.reservation.swabexpress'].search([('name', '=', req['order_number'])])
            else:
            ...
            for provider in req['provider_bookings']:
                provider_obj = self.env['tt.provider.swabexpress'].browse(provider['provider_id'])
                try:
                ...
                if provider['status'] == 'ISSUED':
                    ...
                    book_obj.calculate_service_charge()
                    provider_obj.action_issued_api_swabexpress(context)
                    any_provider_changed = True
    ...
    def get_booking_swabexpress_api(self,req, context):
        try:
    ...
    def payment_swabexpress_api(self,req,context):
        payment_res = self.payment_reservation_api('swabexpress',req,context)
        return payment_res

    def _prepare_booking_api(self, booking_data, context_gateway):
        dest_obj = self.env['tt.destinations']
        provider_type_id = self.env.ref('tt_reservation_swabexpress.tt_provider_type_swabexpress')
        ...
        timeslot_write_data = self.env['tt.timeslot.swabexpress'].search([('seq_id','in',booking_data['timeslot_list'])])
    ...
    def check_provider_state(self,context,pnr_list=[],hold_date=False,req={}):
        ...
        if all(rec.state == 'issued' for rec in self.provider_booking_ids):
            ...
            self.action_reverse_swabexpress(context)
        ...
        for idx, provider in enumerate(self.provider_booking_ids):
            ...
            for p_type,p_val in sc_value.items():
                for c_type,c_val in p_val.items():
                    ...
                    curr_dict = {
                        'pax_type': p_type,
                        'booking_swabexpress_id': self.id,
    ...
    def print_eticket(self, data, ctx=None):
        ...
        datas['form'] = res
        swabexpress_ticket_id = self.env.ref('tt_report_common.action_report_printout_reservation_swabexpress')
        ...
        if not book_obj.printout_ticket_id or data.get('is_hide_agent_logo', False) or data.get('is_force_get_new_printout', False):
            ...
            pdf_report = swabexpress_ticket_id.report_action(book_obj, data=datas)
            pdf_report['context'].update({
                'active_model': book_obj._name,
                'active_id': book_obj.id
            })
            pdf_report_bytes = swabexpress_ticket_id.render_qweb_pdf(data=pdf_report)
    ...
    def print_ho_invoice(self):
        ...
        res = res and res[0] or {}
        datas['form'] = res
        swabexpress_ho_invoice_id = self.env.ref('tt_report_common.action_report_printout_invoice_ho_swabexpress')
        if not self.printout_ho_invoice_id:
            ...
            pdf_report = swabexpress_ho_invoice_id.report_action(self, data=datas)
            pdf_report['context'].update({
                'active_model': self._name,
                'active_id': self.id
            })
            pdf_report_bytes = swabexpress_ho_invoice_id.render_qweb_pdf(data=pdf_report)
    ...
    def print_itinerary(self, data, ctx=None):
        ...
        res = res and res[0] or {}
        datas['form'] = res
        swabexpress_itinerary_id = book_obj.env.ref('tt_report_common.action_printout_itinerary_swabexpress')
        ...
        if not book_obj.printout_itinerary_id or data.get('is_force_get_new_printout', False):
            ...
            pdf_report = swabexpress_itinerary_id.report_action(book_obj, data=datas)
            pdf_report['context'].update({
                'active_model': book_obj._name,
                'active_id': book_obj.id
            })
            pdf_report_bytes = swabexpress_itinerary_id.render_qweb_pdf(data=pdf_report)
            res = book_obj.env['tt.upload.center.wizard'].upload_file_api(
                {
    ...
    def print_itinerary_price(self, data, ctx=None):
        ...
        if not book_obj.printout_itinerary_price_id or data.get('is_force_get_new_printout', False):
            ...
            res = book_obj.env['tt.upload.center.wizard'].upload_file_api(
                {
                    'filename': 'SwabExpress Itinerary %s (Price).pdf' % book_obj.name,
                    'file_reference': 'Itinerary',
                    'file': base64.b64encode(pdf_report_bytes[0]),
                    'delete_date': datetime.today() + timedelta(minutes=10)
                },
    ...
    def confirm_order(self, req, context):
        try:
            provider_obj = self.env['tt.provider.swabexpress'].search([('pnr', '=', req['pnr'])], limit=1)
            if provider_obj:
                ...
                if book_obj.state_vendor == 'new_order':
                    ...
                    for analyst_req_dict in req['analysts']:
                        analyst_obj = self.env['tt.analyst.swabexpress'].create({
