...
class CustomerReportPerformance(models.Model):
    ...
    def _lines(self, date_from, date_to, agent_id, provider_type):
        query = "SELECT {} FROM {} WHERE {} ORDER BY {}".format(self._select(), self._from(provider_type), self._where(date_from, date_to, agent_id, provider_type), self._order_by())
