...
class TtSplitReservationSentraMedikaWizard(models.TransientModel):
    _name = "tt.split.reservation.sentramedika.wizard"
    _description = 'Sentra Medika Split Reservation Wizard'
    ...
    passenger_ids = fields.Many2many('tt.reservation.passenger.sentramedika', 'sentramedika_passenger_split_rel', 'split_wizard_id', 'passenger_id', 'Passenger',
    ...
    def submit_split_reservation(self):
        if not self.env.user.has_group('tt_base.group_reservation_level_4'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 280')
        book_obj = self.env['tt.reservation.sentramedika'].sudo().browse(int(self.res_id))
        pax_list = []
        is_pax_full = True
        ...
        new_book_obj = self.env['tt.reservation.sentramedika'].create(new_vals)
        new_book_obj.write({
        ...
        for rec in book_obj.provider_booking_ids:
            ...
            for rec2 in rec.cost_service_charge_ids:
                ...
                for prov_pax in rec2.passenger_sentramedika_ids:
                    if prov_pax.id in self.passenger_ids.ids:
                        ...
                        if rec2.id not in old_cost_list:
                            cost_val = {
                                ...
                                'passenger_sentramedika_ids': [(4, prov_pax.id)]
                            }
                        ...
                        cost_write_vals = {
                            'passenger_sentramedika_ids': [(3, prov_pax.id)],
                        ...
                        if (rec2.pax_count - 1) <= 0:
                            cost_write_vals.update({
                                'provider_sentramedika_booking_id': False
                            })
            ...
            if rec.id not in old_provider_list:
                ...
                new_prov_obj = self.env['tt.provider.sentramedika'].sudo().create(prov_val)
                ...
                for val in old_cost_dict.values():
                    ...
                    dict_cost_obj.sudo().write({
                        'provider_sentramedika_booking_id': new_prov_obj.id,
                        'description': new_prov_obj.pnr
                    })
            else:
                new_prov_obj = self.env['tt.provider.sentramedika'].sudo().browse(int(provider_dict[str(rec.id)]))
                for val in old_cost_dict.values():
                    dict_cost_obj = self.env['tt.service.charge'].sudo().browse(int(val))
                    dict_cost_obj.sudo().write({
                        'provider_sentramedika_booking_id': new_prov_obj.id,
