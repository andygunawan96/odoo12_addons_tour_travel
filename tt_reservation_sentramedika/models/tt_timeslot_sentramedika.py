...
class TtTimeslotSentraMedika(models.Model):
    ...
    booking_ids = fields.Many2many('tt.reservation.sentramedika','tt_reservation_sentramedika_timeslot_rel', 'timeslot_id', 'booking_id', 'Selected on By Customer Booking(s)')

    antigen_price_ids = fields.Many2many('tt.price.list.sentramedika','tt_price_list_sentramedika_price_antigen_rel','timeslot_antigen_id', 'price_list_id', 'Antigen')

    pcr_price_ids = fields.Many2many('tt.price.list.sentramedika','tt_price_list_sentramedika_price_pcr_rel','timeslot_pcr_id', 'price_list_id', 'PCR')

    mcu_price_ids = fields.Many2many('tt.price.list.sentramedika','tt_price_list_sentramedika_price_mcu_rel','timeslot_mcu_id', 'price_list_id', 'MCU')
    ...
    booking_used_ids = fields.One2many('tt.reservation.sentramedika','picked_timeslot_id', 'Confirmed to Customer Booking(s)')
    ...
    def create(self, vals_list):
        vals_list['seq_id'] = self.env['ir.sequence'].next_by_code('tt.timeslot.sentramedika')
        return super(TtTimeslotSentraMedika, self).create(vals_list)
    ...
    def get_config_cron(self):
        result = self.env['tt.sentramedika.api.con'].get_config_cron()
        return result
...
class TtTimeslotsentramedikadefault(models.Model):
    ...
    antigen_price_ids = fields.Many2many('tt.price.list.sentramedika', 'tt_price_list_sentramedika_price_antigen_default_rel',
                                         'timeslot_antigen_id', 'price_list_id', 'Antigen')

    pcr_price_ids = fields.Many2many('tt.price.list.sentramedika', 'tt_price_list_sentramedika_price_pcr_default_rel',
                                     'timeslot_pcr_id', 'price_list_id', 'PCR')

    mcu_price_ids = fields.Many2many('tt.price.list.sentramedika',
                                              'tt_price_list_sentramedika_price_mcu_default_rel',
                                              'timeslot_mcu_id', 'price_list_id', 'PCR Priority')
