...
class TtCronLogInhSentraMedika(models.Model):
    _inherit = 'tt.cron.log'

    # def cron_reverse_issued_pending(self):
    #     try:
    #         issued_pending_bookings = self.env['tt.reservation.sentramedika'].search(
    ...
    def cron_auto_done_state_vendor_sentramedika(self):
        try:
            issued_bookings = self.env['tt.reservation.sentramedika'].search(
                [('state','=','issued'),
            ...
            self.create_cron_log_folder()
            self.write_cron_log('auto done state vendor sentramedika')

    def cron_auto_create_timeslot_sentramedika(self):
        try:
            time_string = ''
            ...
            for rec in self.env['tt.destinations'].search(
                    [('provider_type_id','=',self.env.ref('tt_reservation_sentramedika.tt_provider_type_sentramedika').id)]):

                # CREATE
                wiz_obj = self.env['create.timeslot.sentramedika.wizard'].create({
            ...
            self.create_cron_log_folder()
            self.write_cron_log('auto create timeslot sentramedika')