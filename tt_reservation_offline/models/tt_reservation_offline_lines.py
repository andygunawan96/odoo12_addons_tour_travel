...
class IssuedOfflineLines(models.Model):
    ...
    def action_create_ledger(self, use_point=False, payment_method_use_to_ho=False):
        if not self.is_ledger_created:
            self.write({
                'is_ledger_created': True
            })
            return self.env['tt.ledger'].action_create_ledger(self, use_point=use_point, payment_method_use_to_ho=payment_method_use_to_ho)
