...
class ActivityAssignProductsWizard(models.TransientModel):
    ...
    def assign_to_selected_ho(self):
        ...
        all_activities = self.env['tt.master.activity'].search([('provider_id', '=', self.provider_id.id),('ho_ids', '!=', self.ho_id.id)])
        for idx, rec in enumerate(all_activities):
            rec.write({
    ...
    def assign_to_multiple_hos(self):
        ...
        for rec in self.ho_ids:
            all_activities = self.env['tt.master.activity'].search([('provider_id', '=', self.provider_id.id),('ho_ids', '!=', rec.id)])
            for idx, rec2 in enumerate(all_activities):
                rec2.write({
                    'ho_ids': [(4, rec.id)]
                })
                _logger.info('Assigning Activity %s to HO %s' % (rec2.name, rec.name))
                if (idx + 1) % 100 == 0:
                    self.env.cr.commit()
    ...
    def delete_from_multiple_hos(self):
        ...
        for rec in self.ho_ids:
            ...
            for idx, rec2 in enumerate(all_activities):
                ...
                _logger.info('Removing Activity %s from HO %s' % (rec2.name, rec.name))
                if (idx + 1) % 100 == 0:
                    self.env.cr.commit()
