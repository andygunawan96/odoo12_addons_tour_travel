...
class ForceIssuedWizard(models.TransientModel):
    _name = "force.issued.wizard.activity"
    _description = 'Activity Force Issued Wizard'
    ...
    def _compute_booker_agent_id(self):
        for rec in self:
            provider_obj = self.env['tt.provider.activity'].search([('id', '=', rec.provider_id.id)])
    ...
    def submit_force_issued(self):
        if not self.env.user.has_group('tt_base.group_reservation_provider_level_4'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 100')
        ...
        provider_obj = self.env['tt.provider.activity'].search([('id', '=', self.provider_id.id)])
        provider_obj.action_force_issued_from_button(payment_data)

    def submit_set_to_issued(self):
        if not self.env.user.has_group('tt_base.group_reservation_provider_level_4'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 101')
        if self.customer_parent_type_id.id != self.env.ref('tt_base.customer_type_fpo').id:
            payment_data = {
                'member': self.use_credit_limit,
                'acquirer_seq_id': self.use_credit_limit and self.customer_parent_id.seq_id or self.acquirer_id.seq_id
