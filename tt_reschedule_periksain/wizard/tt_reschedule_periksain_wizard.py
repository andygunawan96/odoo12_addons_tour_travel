...
class TtReschedulePeriksainWizard(models.TransientModel):
    _name = "tt.reschedule.periksain.wizard"
    ...
    new_picked_timeslot_id = fields.Many2one('tt.timeslot.periksain', 'New Picked Timeslot', domain=[('datetimeslot', '>=', datetime.now())])
    passenger_ids = fields.Many2many('tt.reservation.passenger.periksain', 'tt_reschedule_periksain_wizard_passenger_rel', 'reschedule_id',
    ...
    def submit_reschedule(self):
        ...
        for rec in self.passenger_ids:
            passenger_list.append(rec.id)
        reschedule_obj = self.env['tt.reschedule.periksain'].sudo().create({
        ...
        action_num = self.env.ref('tt_reschedule_periksain.tt_reschedule_periksain_action').id
        menu_num = self.env.ref('tt_reschedule_periksain.menu_transaction_reschedule_periksain').id
        return {
            'type': 'ir.actions.act_url',
            'name': reschedule_obj.name,
            'target': 'self',
            'url': base_url + "/web#id=" + str(
                reschedule_obj.id) + "&action=" + str(action_num) + "&model=tt.reschedule.periksain&view_type=form&menu_id=" + str(menu_num),
        }
