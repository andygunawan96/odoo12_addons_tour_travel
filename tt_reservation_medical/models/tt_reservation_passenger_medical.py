...
class TtReservationCustomer(models.Model):
    _name = 'tt.reservation.passenger.medical'
    _inherit = 'tt.reservation.passenger'
    _description = 'Reservation Passenger medical'

    seq_id = fields.Char('Sequence ID', index=True, readonly=True)

    cost_service_charge_ids = fields.Many2many('tt.service.charge','tt_reservation_medical_cost_charge_rel', 'passenger_id', 'service_charge_id', 'Cost Service Charges')

    channel_service_charge_ids = fields.Many2many('tt.service.charge','tt_reservation_medical_channel_charge_rel', 'passenger_id', 'service_charge_id', 'Channel Service Charges')

    booking_id = fields.Many2one('tt.reservation.medical', 'Booking')
    ...
    def create(self, vals_list):
        vals_list['seq_id'] = self.env['ir.sequence'].next_by_code('tt.reservation.passenger.medical')
    ...
    ## find duplicate passenger that has been sent to Medical but not yet verified
    ...
    def fill_seq_id(self):
        for idx,rec in enumerate(self.search([('seq_id','=',False)])):
            rec.seq_id = "PGM.O%s%s" % (idx,datetime.now().second)
