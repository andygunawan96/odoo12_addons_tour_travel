...
class TtTrainApiCon(models.Model):
    _name = 'tt.swab.express.api.con'
    _inherit = 'tt.api.con'

    table_name = 'tt.reservation.swab.express'

    def action_call(self,table_obj,action,data,context):
        if action == 'get_config':
            res = self.env['tt.provider.swab.express'].get_carriers_api()
        elif action == 'get_availability':
            res = self.env['tt.timeslot.swab.express'].get_available_timeslot_api(context)
        elif action == 'get_price':
            res = table_obj.get_price_swab_express_api(data,context)
        elif action == 'create_booking':
            res = table_obj.create_booking_swab_express_api(data,context)
        elif action == 'update_pnr_provider':
            res = table_obj.update_pnr_provider_swab_express_api(data,context)
        elif action == 'get_booking':
            res = table_obj.get_booking_swab_express_api(data,context)
        elif action == 'confirm_order':
            res = table_obj.confirm_order_api(data,context)
        elif action == 'payment':
            res = table_obj.payment_swab_express_api(data,context)
