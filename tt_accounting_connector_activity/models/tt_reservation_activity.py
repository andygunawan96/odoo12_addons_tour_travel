...
class TtReservationActivity(models.Model):
    ...
    def action_reverse_activity(self,context):
        ...
        if old_state == 'issued':
            temp_post = self.posted_acc_actions or ''
            setup_list = self.env['tt.accounting.setup'].search([('cycle', '=', 'real_time'), ('is_send_activity', '=', True), ('is_send_reverse_transaction', '=', True)])
            if setup_list:
                vendor_list = []
                for rec in setup_list:
                    if rec.accounting_provider not in vendor_list:
        ...
        for rec in transaction_list:
            ...
            if 'reconcile' not in temp_post.split(',') and 'transaction_batch' not in temp_post.split(','):
                setup_list = self.env['tt.accounting.setup'].search(
                    [('cycle', '=', 'per_batch'), ('is_recon_only', '=', False), ('is_send_activity', '=', True)])
                if setup_list:
                    vendor_list = []
