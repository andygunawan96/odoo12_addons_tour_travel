...
class TtProviderSwabExpress(models.Model):
    _name = 'tt.provider.swab.express'
    _inherit = 'tt.history'
    _rec_name = 'pnr'
    ...
    booking_id = fields.Many2one('tt.reservation.swab.express', 'Order Number', ondelete='cascade')
    ...
    ticket_ids = fields.One2many('tt.ticket.swab.express', 'provider_id', 'Ticket Number')
    ...
    sid_cancel = fields.Char('SID Cancel', readonly=True,
                             states={'draft': [('readonly', False)]})  # signature generate sendiri
    cost_service_charge_ids = fields.One2many('tt.service.charge', 'provider_swab_express_booking_id', 'Cost Service Charges')
    ...
    error_history_ids = fields.One2many('tt.reservation.err.history', 'res_id', 'Error History',
                                        domain=[('res_model', '=', 'tt.provider.swab.express')])
    ...
    def action_force_issued_from_button(self, payment_data={}):
        ...
        payment_res = self.booking_id.payment_reservation_api('swab.express', req, context)
    ...
    # def action_booked_api_swab_express(self, provider_data, api_context):
    #     for rec in self:
    #         rec.write({
    ...
    def action_issued_api_swab_express(self, context):
    ...
    def action_issued_swab_express(self, co_uid):
        for rec in self:
            rec.write({
                'state': 'issued',
                'issued_date': datetime.now(),
    ...
    def action_cancel_api_swab_express(self, context):
        for rec in self:
            rec.write({
                'state': 'cancel',
    ...
    def action_cancel_pending_api_swab_express(self, context):
        for rec in self:
            rec.write({
    ...
    def action_failed_booked_api_swab_express(self, err_code, err_msg):
        for rec in self:
            rec.write({
                'state': 'fail_booked',
    ...
    def action_failed_issued_api_swab_express(self, err_code, err_msg):
        for rec in self:
            rec.write({
                'state': 'fail_issued',
                'error_history_ids': [(0, 0, {
    ...
    def action_failed_paid_api_swab_express(self, err_code, err_msg):
        for rec in self:
            rec.write({
                'state': 'fail_paid',
                'error_history_ids': [(0, 0, {
    ...
    def action_failed_void_api_swab_express(self, err_code, err_msg):
        for rec in self:
            rec.write({
                'state': 'void_failed',
                'error_history_ids': [(0, 0, {
    ...
    def create_service_charge(self, service_charge_vals):
        ...
        for scs in service_charge_vals:
            ...
            # scs['pax_count'] = 0
            scs_pax_count = 0
            scs['passenger_swab_express_ids'] = []
            scs['total'] = 0
            ...
            scs['provider_swab_express_booking_id'] = self.id
            for psg in self.ticket_ids:
                if scs['pax_type'] == psg.pax_type and scs_pax_count < scs['pax_count']:
                    scs['passenger_swab_express_ids'].append(psg.passenger_id.id)
                    # scs['pax_count'] += 1
                    scs_pax_count += 1
                    scs['total'] += scs['amount']
            scs['passenger_swab_express_ids'] = [(6, 0, scs['passenger_swab_express_ids'])]
