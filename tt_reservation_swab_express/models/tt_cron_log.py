...
class TtCronLogInhSwabExpress(models.Model):
    ...
    #     try:
    #         issued_pending_bookings = self.env['tt.reservation.swab.express'].search(
    ...
    def cron_auto_done_state_vendor_swab_express(self):
        try:
            issued_bookings = self.env['tt.reservation.swab.express'].search(
                [('state','=','issued'),
            ...
            self.create_cron_log_folder()
            self.write_cron_log('auto done state vendor swab_express')
    ...
    def cron_auto_create_timeslot_swab_express(self):
        try:
            ...
            self.create_cron_log_folder()
            self.write_cron_log('auto create timeslot swab_express')