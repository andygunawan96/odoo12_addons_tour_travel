...
class CreateTimeslotSwabExpressWizard(models.TransientModel):
    _name = "create.timeslot.swab.express.wizard"
    ...
    antigen_price_ids = fields.Many2many('tt.price.list.swab.express', 'tt_price_list_swab_express_price_wizard_antigen_rel','timeslot_antigen_wizard_id', 'price_list_id', 'Antigen')

    pcr_price_ids = fields.Many2many('tt.price.list.swab.express', 'tt_price_list_swab_express_price_wizard_pcr_rel','timeslot_pcr_wizard_id', 'price_list_id', 'PCR')

    pcr_priority_price_ids = fields.Many2many('tt.price.list.swab.express', 'tt_price_list_swab_express_price_wizard_pcr_priority_rel','timeslot_pcr_priority_wizard_id', 'price_list_id', 'PCR Priority')
    ...
    default_data_id = fields.Many2one('tt.timeslot.swab.express.default', 'Default Data')
    ...
    def _get_area_id_domain(self):
        return [('provider_type_id','=',self.env.ref('tt_reservation_swab_express.tt_provider_type_swab_express').id)]
    ...
    def generate_timeslot(self):
        ...
        id_timelist_swab_express = self.id_time_vendor.split(',')
        ...
        time_objs = []
        time_objs_id_swab_express = []
        for idx, time_str in enumerate(timelist):
            time_objs.append((datetime.strptime(time_str,'%H:%M') - timedelta(hours=7)).time())
            time_objs_id_swab_express.append(id_timelist_swab_express[idx] if len(id_timelist_swab_express) > idx else id_timelist_swab_express[0])

        db = self.env['tt.timeslot.swab.express'].search([('destination_id','=',self.area_id.id), ('dateslot','>=',self.start_date), ('dateslot','<=',self.end_date), ('timeslot_type','=',self.timeslot_type), ('agent_id','=',self.agent_id.id if self.agent_id else False)])
        ...
        self.env['tt.timeslot.swab.express'].create(create_values)