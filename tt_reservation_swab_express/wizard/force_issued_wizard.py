...
class ForceIssuedWizard(models.TransientModel):
    _name = "force.issued.wizard.swab.express"
    _description = 'Swab Express Force Issued Wizard'

    provider_id = fields.Many2one('tt.provider.swab.express', 'Provider', readonly=True)
    ...
    def _compute_booker_agent_id(self):
        for rec in self:
            provider_obj = self.env['tt.provider.swab.express'].sudo().search([('id', '=', rec.provider_id.id)])
    ...
    def submit_force_issued(self):
        ...
        provider_obj = self.env['tt.provider.swab.express'].search([('id', '=', self.provider_id.id)])
