...
class AgentReportPassportXls(models.TransientModel):
    ...
    def _print_report_excel(self, data):
        ...
        sheet.write('D9', 'Passport Type', style.table_head_center)
        sheet.write('E9', 'Departure Date', style.table_head_center)
        sheet.write('F9', 'Immigration Consulate', style.table_head_center)
        sheet.write('G9', 'Passenger Name', style.table_head_center)
        sheet.write('H9', 'Passenger Age', style.table_head_center)
        sheet.write('I9', 'Issued By', style.table_head_center)
        sheet.write('J9', 'Issued Date', style.table_head_center)
        sheet.write('K9', 'In Process Date', style.table_head_center)
        sheet.write('L9', 'Done Date', style.table_head_center)
        sheet.write('M9', 'Commission', style.table_head_center)
        sheet.write('N9', 'Grand Total', style.table_head_center)
        sheet.write('O9', 'NTA Amount', style.table_head_center)
        sheet.write('P9', 'State', style.table_head_center)
        sheet.write('Q9', 'State Passport', style.table_head_center)
        ...
        for rec in values['lines']:
            ...
            sheet.write(row_data, 3, rec['passport_type'], sty_table_data_center)  # Passport Type
            sheet.write(row_data, 4,
                        datetime.strptime(rec['departure_date'][:10], "%Y/%m/%d") if rec['departure_date'] else '',
                        sty_date)  # Departure Date
            sheet.write(row_data, 5, rec['immigration_consulate'], sty_table_data)  # Immigration Consulate
            sheet.write(row_data, 6, rec['pass_name'], sty_table_data)  # Passenger Name
            sheet.write(row_data, 7, rec['age'], sty_table_data)  # Passenger Age | rec['age']
            sheet.write(row_data, 8, rec['issued_name'], sty_table_data)  # Issued By
            if rec['issued_date']:  # Issued Date
                sheet.write(row_data, 9, rec['issued_date'].strftime("%Y-%m-%d %H:%M:%S"), sty_datetime)
            else:
                sheet.write(row_data, 9, '', sty_datetime)
            if rec['in_process_date']:  # In Process Date
                sheet.write(row_data, 10, rec['in_process_date'].strftime("%Y-%m-%d %H:%M:%S"), sty_datetime)
            else:
                sheet.write(row_data, 10, '', sty_datetime)
            if rec['done_date']:  # Done Date
                sheet.write(row_data, 11, rec['done_date'].strftime("%Y-%m-%d %H:%M:%S"), sty_datetime)
            else:
                sheet.write(row_data, 11, '', sty_datetime)
            sheet.write(row_data, 12, rec['commission'], sty_table_data_center)  # Commission
            sheet.write(row_data, 13, rec['total'], sty_table_data_center)  # Grand Total
            sheet.write(row_data, 14, rec['total_nta'], sty_table_data_center)  # NTA Amount
            sheet.write(row_data, 15, BOOKING_STATE_STR[rec['state']] if rec['state'] else '', sty_table_data_center)  # State
            sheet.write(row_data, 16, rec['state_passport'] if rec['state_passport'] else '',
                        sty_table_data_center)  # State Passport
