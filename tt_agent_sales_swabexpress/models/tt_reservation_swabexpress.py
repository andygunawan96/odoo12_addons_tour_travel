...
class ReservationSwabExpress(models.Model):
    ...
    _inherit = 'tt.reservation.swabexpress'
    ...
    invoice_line_ids = fields.One2many('tt.agent.invoice.line', 'res_id_resv', 'Invoice',
                                       domain=[('res_model_resv', '=', 'tt.reservation.swabexpress')])
    ...
    def action_reverse_swabexpress(self,context):
        super(ReservationSwabExpress, self).action_reverse_swabexpress(context)
        for rec in self.invoice_line_ids:
            try:
    ...
    def action_issued_swabexpress(self,co_uid,customer_parent_id,acquirer_id):
        super(ReservationSwabExpress, self).action_issued_swabexpress(co_uid,customer_parent_id)
