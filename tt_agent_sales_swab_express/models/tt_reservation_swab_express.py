...
class ReservationSwabExpress(models.Model):
    ...
    _inherit = 'tt.reservation.swab.express'
    ...
    invoice_line_ids = fields.One2many('tt.agent.invoice.line', 'res_id_resv', 'Invoice',
                                       domain=[('res_model_resv', '=', 'tt.reservation.lab.pintar')])
    ...
    def action_reverse_swab_express(self,context):
        super(ReservationSwabExpress, self).action_reverse_swab_express(context)
        for rec in self.invoice_line_ids:
            try:
    ...
    def action_issued_swab_express(self,co_uid,customer_parent_id,acquirer_id):
        super(ReservationSwabExpress, self).action_issued_swab_express(co_uid,customer_parent_id)
