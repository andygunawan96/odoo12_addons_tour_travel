...
class ReservationLabPintar(models.Model):
    _name = "tt.reservation.lab.pintar"
    ...
    sale_service_charge_ids = fields.One2many('tt.service.charge', 'booking_lab_pintar_id', 'Service Charge',
                                              readonly=True, states={'draft': [('readonly', False)]})

    passenger_ids = fields.One2many('tt.reservation.passenger.lab.pintar', 'booking_id',
                                    readonly=True, states={'draft': [('readonly', False)]})
    ...
    provider_booking_ids = fields.One2many('tt.provider.lab.pintar', 'booking_id', string='Provider Booking', readonly=True, states={'draft': [('readonly', False)]})

    timeslot_ids = fields.Many2many('tt.timeslot.lab.pintar','tt_reservation_lab_pintar_timeslot_rel', 'booking_id', 'timeslot_id', 'Timeslot(s)')

    picked_timeslot_id = fields.Many2one('tt.timeslot.lab.pintar', 'Picked Timeslot', readonly=True, states={'draft': [('readonly', False)]})

    test_datetime = fields.Datetime('Test Datetime',related='picked_timeslot_id.datetimeslot', store=True)

    analyst_ids = fields.Many2many('tt.analyst.lab.pintar', 'tt_reservation_lab_pintar_analyst_rel', 'booking_id',
                                   'analyst_id', 'Analyst(s)', readonly=True, states={'draft': [('readonly', False)]})

    provider_type_id = fields.Many2one('tt.provider.type','Provider Type',
                                       default=lambda self: self.env.ref('tt_reservation_lab_pintar.tt_provider_type_lab_pintar'))

    split_from_resv_id = fields.Many2one('tt.reservation.lab.pintar', 'Splitted From', readonly=1)
    split_to_resv_ids = fields.One2many('tt.reservation.lab.pintar', 'split_from_resv_id', 'Splitted To', readonly=1)
    split_uid = fields.Many2one('res.users', 'Splitted by', readonly=True)
    split_date = fields.Datetime('Splitted Date', readonly=True)

    def get_form_id(self):
        return self.env.ref("tt_reservation_lab_pintar.tt_reservation_lab_pintar_form_views")
    ...
    def action_booked_api_lab_pintar(self,context):
        write_values = {
            'state': 'booked',
        ...
        try:
            if self.agent_type_id.is_send_email_booked:
                mail_created = self.env['tt.email.queue'].sudo().with_context({'active_test':False}).search([('res_id', '=', self.id), ('res_model', '=', self._name), ('type', '=', 'booked_lab_pintar')], limit=1)
                if not mail_created:
                    temp_data = {
                        'provider_type': 'lab_pintar',
    ...
    def action_issued_lab_pintar(self, co_uid, customer_parent_id, acquirer_id = False):
        ...
        try:
            if self.agent_type_id.is_send_email_issued:
                mail_created = self.env['tt.email.queue'].sudo().with_context({'active_test': False}).search([('res_id', '=', self.id), ('res_model', '=', self._name), ('type', '=', 'issued_lab_pintar')], limit=1)
                if not mail_created:
                    temp_data = {
                        'provider_type': 'lab_pintar',
                        'order_number': self.name,
    ...
    def action_reverse_lab_pintar(self,context):
        self.write({
            'state':  'fail_refunded',
    ...
    def action_refund_failed_lab_pintar(self,context):
        self.write({
            'state':  'refund_failed',
        })

    def action_issued_api_lab_pintar(self,acquirer_id,customer_parent_id,context):
        self.action_issued_lab_pintar(context['co_uid'],customer_parent_id,acquirer_id)

    # def action_issued_lab_pintar(self,context):
    #     values = {
    #         'state': 'issued',
    #         'issued_date': datetime.now(),
    ...
    #
    #     try:
    #         mail_created = self.env['tt.email.queue'].sudo().with_context({'active_test':False}).search([('res_id', '=', self.id), ('res_model', '=', self._name), ('type', '=', 'issued_final_lab_pintar')], limit=1)
    #         if not mail_created:
    #             temp_data = {
    #                 'provider_type': 'lab_pintar',
    #                 'order_number': self.name,
    ...
    def get_price_lab_pintar_api(self, req, context):
        #parameter
        #timeslot_list
        #jumlah pax
        overtime_surcharge = False
        timeslot_objs = self.env['tt.timeslot.lab.pintar'].search([('seq_id', 'in', req['timeslot_list'])])
        ...
        if req['pax_count'] <= 1 and \
                carrier_id == self.env.ref('tt_reservation_lab_pintar.tt_transport_carrier_lab_pintar_antigen').id:
        ...
        if carrier_id == self.env.ref('tt_reservation_lab_pintar.tt_transport_carrier_lab_pintar_antigen').id:
            for rec in timeslot_objs:
    ...
    def create_booking_lab_pintar_api(self, req, context):
        _logger.info("Create\n" + json.dumps(req))
        booker = req['booker']
    ...
    def update_pnr_provider_lab_pintar_api(self, req, context):
        _logger.info("Update\n" + json.dumps(req))
        try:
            if req.get('book_id'):
                book_obj = self.env['tt.reservation.lab.pintar'].browse(req['book_id'])
            elif req.get('order_number'):
                book_obj = self.env['tt.reservation.lab.pintar'].search([('name', '=', req['order_number'])])
            else:
            ...
            for provider in req['provider_bookings']:
                provider_obj = self.env['tt.provider.lab.pintar'].browse(provider['provider_id'])
                try:
                ...
                if provider['status'] == 'ISSUED':
                    ...
                    book_obj.calculate_service_charge()
                    provider_obj.action_issued_api_lab_pintar(context)
                    any_provider_changed = True
    ...
    def get_booking_lab_pintar_api(self,req, context):
        try:
    ...
    def payment_lab_pintar_api(self,req,context):
        payment_res = self.payment_reservation_api('lab.pintar',req,context)
        return payment_res

    def _prepare_booking_api(self, booking_data, context_gateway):
        dest_obj = self.env['tt.destinations']
        provider_type_id = self.env.ref('tt_reservation_lab_pintar.tt_provider_type_lab_pintar')
        ...
        timeslot_write_data = self.env['tt.timeslot.lab.pintar'].search([('seq_id','in',booking_data['timeslot_list'])])
    ...
    def check_provider_state(self,context,pnr_list=[],hold_date=False,req={}):
        ...
        #     self.action_issued_lab_pintar(context)
        if all(rec.state == 'issued' for rec in self.provider_booking_ids):
            acquirer_id, customer_parent_id = self.get_acquirer_n_c_parent_id(req)
            self.action_issued_api_lab_pintar(acquirer_id and acquirer_id.id or False, customer_parent_id, context)
        elif all(rec.state == 'booked' for rec in self.provider_booking_ids):
            self.action_booked_api_lab_pintar(context)
            ...
            self.action_reverse_lab_pintar(context)
    ...
    def print_eticket(self, data, ctx=None):
        ...
        datas['form'] = res
        lab_pintar_ticket_id = self.env.ref('tt_report_common.action_report_printout_reservation_lab_pintar')
        ...
        if not book_obj.printout_ticket_id:
            ...
            pdf_report = lab_pintar_ticket_id.report_action(book_obj, data=datas)
            pdf_report['context'].update({
                'active_model': book_obj._name,
                'active_id': book_obj.id
            })
            pdf_report_bytes = lab_pintar_ticket_id.render_qweb_pdf(data=pdf_report)
    ...
    def print_ho_invoice(self):
        ...
        res = res and res[0] or {}
        datas['form'] = res
        lab_pintar_ho_invoice_id = self.env.ref('tt_report_common.action_report_printout_invoice_ho_airline')
        if not self.printout_ho_invoice_id:
            ...
            pdf_report = lab_pintar_ho_invoice_id.report_action(self, data=datas)
            pdf_report['context'].update({
                'active_model': self._name,
                'active_id': self.id
            })
            pdf_report_bytes = lab_pintar_ho_invoice_id.render_qweb_pdf(data=pdf_report)
    ...
    def print_itinerary(self, data, ctx=None):
        ...
        datas['form'] = res
        datas['is_with_price'] = True
        lab_pintar_itinerary_id = book_obj.env.ref('tt_report_common.action_printout_itinerary_lab_pintar')
        ...
        if not book_obj.printout_ho_invoice_id:
            ...
            pdf_report = lab_pintar_itinerary_id.report_action(book_obj, data=datas)
            pdf_report['context'].update({
                'active_model': book_obj._name,
                'active_id': book_obj.id
            })
            pdf_report_bytes = lab_pintar_itinerary_id.render_qweb_pdf(data=pdf_report)
            res = book_obj.env['tt.upload.center.wizard'].upload_file_api(
                {
    ...
    def get_terms_conditions_email(self):
        if self.carrier_name == 'LPKATG':
            template_obj = self.env.ref('tt_report_common.lab_pintar_antigen_information')
        elif self.carrier_name == 'LPKPCR':
            template_obj = self.env.ref('tt_report_common.lab_pintar_pcr_information')
        else:
            template_obj = self.env.ref('tt_report_common.lab_pintar_pcr_priority_information')
    ...
    def confirm_order(self, req, context):
        try:
            provider_obj = self.env['tt.provider.lab.pintar'].search([('pnr', '=', req['pnr'])], limit=1)
            if provider_obj:
                ...
                if book_obj.state_vendor == 'new_order':
                    ...
                    for analyst_req_dict in req['analysts']:
                        analyst_obj = self.env['tt.analyst.lab.pintar'].create({
