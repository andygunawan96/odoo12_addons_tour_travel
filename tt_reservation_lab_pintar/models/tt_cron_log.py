...
class TtCronLogInhLabPintar(models.Model):
    ...
    #     try:
    #         issued_pending_bookings = self.env['tt.reservation.lab.pintar'].search(
    ...
    def cron_auto_done_state_vendor_lab_pintar(self):
        try:
            issued_bookings = self.env['tt.reservation.lab.pintar'].search(
                [('state','=','issued'),
    ...
    def cron_auto_create_timeslot_lab_pintar(self):
        try:
            res = self.env['tt.timeslot.lab.pintar'].get_config_cron()
            kota_list = []
            jenis_tindakan = {}
            time_string = ''
            id_time_string = ''
            ...
            for rec in self.env['tt.destinations'].search(
                    [('provider_type_id','=',self.env.ref('tt_reservation_lab_pintar.tt_provider_type_lab_pintar').id)]):
                for kota in res['response']['kota']:
                    if rec.name in kota['kabupaten']['nama']:
                        rec.icao = "%s~%s" % (kota['id'], json.dumps(jenis_tindakan))
                        # CREATE
                        wiz_obj = self.env['create.timeslot.lab.pintar.wizard'].create({
            ...
            self.create_cron_log_folder()
            self.write_cron_log('auto create timeslot lab_pintar')