...
class CreateTimeslotLabPintarWizard(models.TransientModel):
    _name = "create.timeslot.lab.pintar.wizard"
    ...
    antigen_price_ids = fields.Many2many('tt.price.list.lab.pintar', 'tt_price_list_lab_pintar_price_wizard_antigen_rel','timeslot_antigen_wizard_id', 'price_list_id', 'Antigen')
    ...
    default_data_id = fields.Many2one('tt.timeslot.lab.pintar.default', 'Default Data')
    ...
    def _get_area_id_domain(self):
        return [('provider_type_id','=',self.env.ref('tt_reservation_lab_pintar.tt_provider_type_lab_pintar').id)]
    ...
    def generate_timeslot(self):
        ...
        id_timelist_lab_pintar = self.id_time_vendor.split(',')
        ...
        time_objs = []
        time_objs_id_lab_pintar = []
        for idx, time_str in enumerate(timelist):
            time_objs.append((datetime.strptime(time_str,'%H:%M') - timedelta(hours=7)).time())
            time_objs_id_lab_pintar.append(id_timelist_lab_pintar[idx] if len(id_timelist_lab_pintar) > idx else id_timelist_lab_pintar[0])

        db = self.env['tt.timeslot.lab.pintar'].search([('destination_id','=',self.area_id.id), ('dateslot','>=',self.start_date), ('dateslot','<=',self.end_date), ('timeslot_type','=',self.timeslot_type), ('agent_id','=',self.agent_id.id if self.agent_id else False)])
        ...
        self.env['tt.timeslot.lab.pintar'].create(create_values)