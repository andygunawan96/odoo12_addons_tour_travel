...
class ForceIssuedWizard(models.TransientModel):
    _name = "force.issued.wizard.lab.pintar"
    _description = 'Lab Pintar Force Issued Wizard'

    provider_id = fields.Many2one('tt.provider.lab.pintar', 'Provider', readonly=True)
    ...
    def _compute_booker_agent_id(self):
        for rec in self:
            provider_obj = self.env['tt.provider.lab.pintar'].sudo().search([('id', '=', rec.provider_id.id)])
    ...
    def submit_force_issued(self):
        ...
        provider_obj = self.env['tt.provider.lab.pintar'].search([('id', '=', self.provider_id.id)])
