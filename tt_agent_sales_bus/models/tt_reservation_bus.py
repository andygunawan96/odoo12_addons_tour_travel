...
class ReservationBus(models.Model):

    _inherit = 'tt.reservation.bus'
    ...
    invoice_line_ids = fields.One2many('tt.agent.invoice.line', 'res_id_resv', 'Invoice',
                                       domain=[('res_model_resv', '=', 'tt.reservation.bus')])
    ...
    #     res = super(ReservationBus, self).action_check_provider_state(api_context)
    ...
    def action_reverse_bus(self,context):
        super(ReservationBus, self).action_reverse_bus(context)
        for rec in self.invoice_line_ids:
            try:
    ...
    def action_issued_bus(self,co_uid,customer_parent_id,acquirer_id):
        super(ReservationBus, self).action_issued_bus(co_uid,customer_parent_id)
