...
class TtCronLogInhResv(models.Model):
    ...
    def cron_mail_hotel_confirmation(self):
        ...
        for ho_obj in ho_objs:
            try:
                for to_send in self.env['tt.reservation.hotel'].sudo().search([('create_date', '<=', datetime.today() - timedelta(days=1)),('create_date', '>', datetime.today() - timedelta(days=2)),('provider_name', 'in', list_provider),('ho_id','=',ho_obj.id)]):
                ...
                self.write_cron_log('Update ' + mail_type, ho_id=ho_obj.id)
    ...
    def cron_mail_hotel_retrieve_booking(self):
        ...
        for ho_obj in ho_objs:
            try:
                for to_send in self.env['tt.reservation.hotel'].sudo().search([('create_date', '<=', datetime.today() - timedelta(minutes=2)),('create_date', '>', datetime.today() - timedelta(minutes=4)),('provider_name', 'in', list_provider),('ho_id','=',ho_obj.id)]):
                    to_send.check_booking_status()

            except Exception as e:
                self.create_cron_log_folder()
                self.write_cron_log('Retrieve Booking Hotel : TBO', ho_id=ho_obj.id)
