...
class ReservationMitraKeluarga(models.Model):
    _name = "tt.reservation.mitrakeluarga"
    _inherit = "tt.reservation"
    _order = "id desc"
    _description = "Reservation Swab Express"

    timeslot_type = fields.Selection([('fixed','Fixed'),
                                      ('flexible','Flexible')],'Time Slot Type',readonly=True, states={'draft': [('readonly', False)]})

    sale_service_charge_ids = fields.One2many('tt.service.charge', 'booking_mitrakeluarga_id', 'Service Charge',
                                              readonly=True, states={'draft': [('readonly', False)]})

    passenger_ids = fields.One2many('tt.reservation.passenger.mitrakeluarga', 'booking_id',
                                    readonly=True, states={'draft': [('readonly', False)]})
    ...
    provider_booking_ids = fields.One2many('tt.provider.mitrakeluarga', 'booking_id', string='Provider Booking', readonly=True, states={'draft': [('readonly', False)]})

    timeslot_ids = fields.Many2many('tt.timeslot.mitrakeluarga','tt_reservation_mitrakeluarga_timeslot_rel', 'booking_id', 'timeslot_id', 'Timeslot(s)')

    picked_timeslot_id = fields.Many2one('tt.timeslot.mitrakeluarga', 'Picked Timeslot', readonly=True, states={'draft': [('readonly', False)]})

    test_datetime = fields.Datetime('Test Datetime',related='picked_timeslot_id.datetimeslot', store=True)

    analyst_ids = fields.Many2many('tt.analyst.mitrakeluarga', 'tt_reservation_mitrakeluarga_analyst_rel', 'booking_id',
                                   'analyst_id', 'Analyst(s)', readonly=True, states={'draft': [('readonly', False)]})

    provider_type_id = fields.Many2one('tt.provider.type','Provider Type',
                                       default=lambda self: self.env.ref('tt_reservation_mitrakeluarga.tt_provider_type_mitrakeluarga'))

    split_from_resv_id = fields.Many2one('tt.reservation.mitrakeluarga', 'Splitted From', readonly=1)
    split_to_resv_ids = fields.One2many('tt.reservation.mitrakeluarga', 'split_from_resv_id', 'Splitted To', readonly=1)
    split_uid = fields.Many2one('res.users', 'Splitted by', readonly=True)
    split_date = fields.Datetime('Splitted Date', readonly=True)

    def get_form_id(self):
        return self.env.ref("tt_reservation_mitrakeluarga.tt_reservation_mitrakeluarga_form_views")
    ...
    def action_set_as_draft(self):
        if not self.env.user.has_group('base.group_system'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 186')
        for rec in self:
            rec.state = 'draft'
    ...
    def action_set_as_booked(self):
        if not self.env.user.has_group('base.group_system'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 187')
        for rec in self:
            rec.state = 'booked'
    ...
    def action_set_as_issued(self):
        if not self.env.user.has_group('base.group_system'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 188')
        for rec in self:
            rec.state = 'issued'
    ...
    def action_set_state_vendor_as_test_completed(self):
        if not ({self.env.ref('tt_base.group_external_vendor_mitrakeluarga_level_2').id, self.env.ref('tt_base.group_reservation_level_4').id}.intersection(set(self.env.user.groups_id.ids))):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 189')
        self.state_vendor = 'test_completed'

    def action_set_state_vendor_as_no_show(self):
        if not ({self.env.ref('tt_base.group_external_vendor_mitrakeluarga_level_2').id, self.env.ref('tt_base.group_reservation_level_4').id}.intersection(set(self.env.user.groups_id.ids))):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 190')
        self.state_vendor = 'no_show'

    def action_set_state_vendor_as_refund(self):
        if not ({self.env.ref('tt_base.group_external_vendor_mitrakeluarga_level_2').id, self.env.ref('tt_base.group_reservation_level_4').id}.intersection(set(self.env.user.groups_id.ids))):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 191')
        self.state_vendor = 'refund'

    def action_booked_api_mitrakeluarga(self,context):
        write_values = {
            'state': 'booked',
        ...
        try:
            if self.agent_type_id.is_send_email_booked:
                mail_created = self.env['tt.email.queue'].sudo().with_context({'active_test':False}).search([('res_id', '=', self.id), ('res_model', '=', self._name), ('type', '=', 'booked_mitrakeluarga')], limit=1)
                if not mail_created:
                    temp_data = {
                        'provider_type': 'mitrakeluarga',
    ...
    def action_issued_mitrakeluarga(self,data):
        ...
        try:
            if self.agent_type_id.is_send_email_issued:
                mail_created = self.env['tt.email.queue'].sudo().with_context({'active_test': False}).search([('res_id', '=', self.id), ('res_model', '=', self._name), ('type', '=', 'issued_mitrakeluarga')], limit=1)
                if not mail_created:
                    temp_data = {
                        'provider_type': 'mitrakeluarga',
                        'order_number': self.name,
    ...
    def action_refund_failed_mitrakeluarga(self,context):
        self.write({
            'state':  'refund_failed',
        })

    def action_issued_api_mitrakeluarga(self,req,context):
        data = {
        ...
        self.action_issued_mitrakeluarga(data)

    # def action_issued_mitrakeluarga(self,context):
    #     values = {
    #         'state': 'issued',
    #         'issued_date': datetime.now(),
    ...
    #
    #     try:
    #         mail_created = self.env['tt.email.queue'].sudo().with_context({'active_test':False}).search([('res_id', '=', self.id), ('res_model', '=', self._name), ('type', '=', 'issued_final_mitrakeluarga')], limit=1)
    #         if not mail_created:
    #             temp_data = {
    #                 'provider_type': 'mitrakeluarga',
    #                 'order_number': self.name,
    ...
    def action_cancel(self, backend_context=False, gateway_context=False):
        if not self.env.user.has_group('tt_base.group_reservation_level_4'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 192')
        super(ReservationMitraKeluarga, self).action_cancel(gateway_context=gateway_context)
    ...
    def get_price_mitrakeluarga_api(self, req, context):
        #parameter
        #timeslot_list
        #jumlah pax
        overtime_surcharge = False
        timeslot_objs = self.env['tt.timeslot.mitrakeluarga'].search([('seq_id', 'in', req['timeslot_list'])])
        carrier_id = self.env['tt.transport.carrier'].search([('code', '=', req['carrier_code']),('provider_type_id','=',self.env.ref('tt_reservation_mitrakeluarga.tt_provider_type_mitrakeluarga').id)]).id
        ...
        if carrier_id in [self.env.ref('tt_reservation_mitrakeluarga.tt_transport_carrier_mitrakeluarga_drivethru_antigen').id,
                          self.env.ref('tt_reservation_mitrakeluarga.tt_transport_carrier_mitrakeluarga_homecare_antigen').id]:
            for rec in timeslot_objs:
                for antigen_price_pax in rec.antigen_price_ids:
                    if req['pax_count'] >= antigen_price_pax['min_pax']:
                        ...
                        break
        elif carrier_id in [self.env.ref('tt_reservation_mitrakeluarga.tt_transport_carrier_mitrakeluarga_drivethru_pcr').id,
            ...
            for rec in timeslot_objs:
                for srbd_price_pax in rec.srbd_price_ids:
                    if req['pax_count'] >= srbd_price_pax['min_pax']:
                        base_price = srbd_price_pax.base_price
                        commission_price = srbd_price_pax.commission
                        overtime_price = timeslot_objs.overtime_surcharge
                        single_suplement_price = timeslot_objs.single_supplement
                        cito_suplement_price = timeslot_objs.cito_surcharge
                    else:
    ...
    def create_booking_mitrakeluarga_api(self, req, context):
        _logger.info("Create\n" + json.dumps(req))
        booker = req['booker']
    ...
    def update_pnr_provider_mitrakeluarga_api(self, req, context):
        _logger.info("Update\n" + json.dumps(req))
        try:
            if req.get('book_id'):
                book_obj = self.env['tt.reservation.mitrakeluarga'].browse(req['book_id'])
            elif req.get('order_number'):
                book_obj = self.env['tt.reservation.mitrakeluarga'].search([('name', '=', req['order_number'])])
            else:
            ...
            for provider in req['provider_bookings']:
                provider_obj = self.env['tt.provider.mitrakeluarga'].browse(provider['provider_id'])
                try:
                ...
                if provider['status'] == 'ISSUED':
                    ...
                    book_obj.calculate_service_charge()
                    provider_obj.action_issued_api_mitrakeluarga(context)
                    any_provider_changed = True
    ...
    def get_booking_mitrakeluarga_api(self,req, context):
        try:
    ...
    def payment_mitrakeluarga_api(self,req,context):
        payment_res = self.payment_reservation_api('mitrakeluarga',req,context)
        return payment_res
    ...
    def check_provider_state(self,context,pnr_list=[],hold_date=False,req={}):
        ...
        #     self.action_issued_mitrakeluarga(context)
        if all(rec.state == 'issued' for rec in self.provider_booking_ids):
            ...
            self.action_issued_api_mitrakeluarga(issued_req, context)
        elif all(rec.state == 'booked' for rec in self.provider_booking_ids):
            self.action_booked_api_mitrakeluarga(context)
            ...
            self.action_reverse_mitrakeluarga(context)
        ...
        for idx, provider in enumerate(self.provider_booking_ids):
            ...
            for p_type,p_val in sc_value.items():
                for c_type,c_val in p_val.items():
                    ...
                    curr_dict = {
                        'pax_type': p_type,
                        'booking_mitrakeluarga_id': self.id,
    ...
    def print_eticket(self, data, ctx=None):
        ...
        if not book_obj.printout_ticket_id or data.get('is_hide_agent_logo', False) or data.get('is_force_get_new_printout', False):
            ...
            pdf_report = mitrakeluarga_ticket_id.report_action(book_obj, data=datas)
    ...
    def print_ho_invoice(self):
        ...
        res = res and res[0] or {}
        datas['form'] = res
        mitrakeluarga_ho_invoice_id = self.env.ref('tt_report_common.action_report_printout_invoice_ho_mitrakeluarga')
        if not self.printout_ho_invoice_id:
            ...
            pdf_report = mitrakeluarga_ho_invoice_id.report_action(self, data=datas)
            pdf_report['context'].update({
    ...
    def print_itinerary(self, data, ctx=None):
        ...
        if not book_obj.printout_itinerary_id or data.get('is_force_get_new_printout', False):
            ...
            pdf_report = mitrakeluarga_itinerary_id.report_action(book_obj, data=datas)
            pdf_report['context'].update({
                'active_model': book_obj._name,
                'active_id': book_obj.id
            })
            pdf_report_bytes = mitrakeluarga_itinerary_id.render_qweb_pdf(data=pdf_report)
            res = book_obj.env['tt.upload.center.wizard'].upload_file_api(
                {
    ...
    def print_itinerary_price(self, data, ctx=None):
        ...
        if not book_obj.printout_itinerary_price_id or data.get('is_force_get_new_printout', False):
            ...
            res = book_obj.env['tt.upload.center.wizard'].upload_file_api(
                {
                    'filename': 'MitraKeluarga Itinerary %s (Price).pdf' % book_obj.name,
                    'file_reference': 'Itinerary',
                    'file': base64.b64encode(pdf_report_bytes[0]),
                    'delete_date': datetime.today() + timedelta(minutes=10)
                },
    ...
    def get_terms_conditions_email_old(self):
        ...
        if self.carrier_name in ['PRKPCR']:
            ...
            terms_txt += "7. Under normal circumstances, test results will be sent via Email by Swab Express around 2-4 hours after the test.<br/>"
    ...
    def confirm_order(self, req, context):
        try:
            provider_obj = self.env['tt.provider.mitrakeluarga'].search([('pnr', '=', req['pnr'])], limit=1)
            if provider_obj:
                ...
                if book_obj.state_vendor == 'new_order':
                    ...
                    for analyst_req_dict in req['analysts']:
                        analyst_obj = self.env['tt.analyst.mitrakeluarga'].create({
