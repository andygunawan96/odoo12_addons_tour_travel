...
class TtSplitReservationMitraKeluargaWizard(models.TransientModel):
    _name = "tt.split.reservation.mitrakeluarga.wizard"
    _description = 'Swab Express Split Reservation Wizard'
    ...
    passenger_ids = fields.Many2many('tt.reservation.passenger.mitrakeluarga', 'mitrakeluarga_passenger_split_rel', 'split_wizard_id', 'passenger_id', 'Passenger',
    ...
    def submit_split_reservation(self):
        if not self.env.user.has_group('tt_base.group_reservation_level_4'):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 197')
        book_obj = self.env['tt.reservation.mitrakeluarga'].sudo().browse(int(self.res_id))
        pax_list = []
        is_pax_full = True
        ...
        new_book_obj = self.env['tt.reservation.mitrakeluarga'].create(new_vals)
        new_book_obj.write({
        ...
        for rec in book_obj.provider_booking_ids:
            ...
            for rec2 in rec.cost_service_charge_ids:
                ...
                for prov_pax in rec2.passenger_mitrakeluarga_ids:
                    if prov_pax.id in self.passenger_ids.ids:
                        ...
                        if rec2.id not in old_cost_list:
                            cost_val = {
                                ...
                                'passenger_mitrakeluarga_ids': [(4, prov_pax.id)]
                            }
                        ...
                        cost_write_vals = {
                            'passenger_mitrakeluarga_ids': [(3, prov_pax.id)],
                        ...
                        if (rec2.pax_count - 1) <= 0:
                            cost_write_vals.update({
                                'provider_mitrakeluarga_booking_id': False
                            })
            ...
            if rec.id not in old_provider_list:
                ...
                new_prov_obj = self.env['tt.provider.mitrakeluarga'].sudo().create(prov_val)
                ...
                for val in old_cost_dict.values():
                    ...
                    dict_cost_obj.sudo().write({
                        'provider_mitrakeluarga_booking_id': new_prov_obj.id,
                        'description': new_prov_obj.pnr
                    })
            else:
                new_prov_obj = self.env['tt.provider.mitrakeluarga'].sudo().browse(int(provider_dict[str(rec.id)]))
                for val in old_cost_dict.values():
                    dict_cost_obj = self.env['tt.service.charge'].sudo().browse(int(val))
                    dict_cost_obj.sudo().write({
                        'provider_mitrakeluarga_booking_id': new_prov_obj.id,
