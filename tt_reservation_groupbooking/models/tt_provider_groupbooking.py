...
class ProviderGroupBooking(models.Model):
    ...
    def generate_sc_repricing(self):
        ...
        if adt_count > 0:
            scs_list.append({
                ...
                'pax_type': 'ADT',
                'currency_id': self.currency_id.id,
                'provider_groupbooking_booking_id': self.id,
                'passenger_groupbooking_ids': adt_id_list,
                'pax_count': adt_count,
                'total': (sale_price / len(self.booking_id.passenger_ids)) * adt_count,
            })
            adt_scs_list = self.booking_id.calculate_groupbooking_pricing({
                'fare_amount': 0,
                'tax_amount': 0,
                'roc_amount': 0,
        ...
        if chd_count > 0:
            scs_list.append({
                ...
                'currency_id': self.currency_id.id,
                'provider_groupbooking_booking_id': self.id,
                'passenger_groupbooking_ids': chd_id_list,
                'pax_count': chd_count,
                'total': (sale_price / len(self.booking_id.passenger_ids)) * chd_count,
            })
            chd_scs_list = self.booking_id.calculate_groupbooking_pricing({
                'fare_amount': 0,
                'tax_amount': 0,
                'roc_amount': 0,
                'rac_amount': (total_amount / len(self.booking_id.line_ids) * provider_line_count / total_pax_count) * -1,
        ...
        if inf_count > 0:
            scs_list.append({
                ...
                'currency_id': self.currency_id.id,
                'provider_groupbooking_booking_id': self.id,
                'passenger_groupbooking_ids': inf_id_list,
                'pax_count': inf_count,
                'total': (sale_price / len(self.booking_id.passenger_ids)) * inf_count,
            })
            inf_scs_list = self.booking_id.calculate_groupbooking_pricing({
                'fare_amount': 0,
                'tax_amount': 0,
                'roc_amount': 0,
                'rac_amount': (total_amount / len(self.booking_id.line_ids) * provider_line_count / total_pax_count) * -1,
        ...
        for comm in commission_list:
            ...
            if comm['amount'] != 0:
                comm.update({
                    'passenger_groupbooking_ids': psg_id_list,
                    'description': '',
                    'currency_id': self.currency_id.id,
                    'provider_groupbooking_booking_id': self.id
                })
        ...
        if self.booking_id.admin_fee_ho != 0:
            ...
            scs_list.append({
                ...
                'description': '',
                'pax_type': 'ADT',
                'passenger_groupbooking_ids': scs_list[0]['passenger_groupbooking_ids'],
                'provider_groupbooking_booking_id': self.id,
    ...
    def generate_sc_repricing_v2(self, idx):
        ...
        if idx == 1 and self.booking_id.price_pick_departure_id:
            sale_price = self.booking_id.price_pick_departure_id.amount * (self.booking_id.adult + self.booking_id.child + self.booking_id.infant)
            total_commission_amount = self.booking_id.price_pick_departure_id.commission * (self.booking_id.adult + self.booking_id.child + self.booking_id.infant)
        elif idx == 1 and self.booking_id.price_pick_return_id:
            sale_price = self.booking_id.price_pick_return_id.amount * (self.booking_id.adult + self.booking_id.child + self.booking_id.infant)
            total_commission_amount = self.booking_id.price_pick_return_id.commission * (self.booking_id.adult + self.booking_id.child + self.booking_id.infant)
    ...
    def create_service_charge_hotel(self, index):
        ...
        for scs_2 in scs_list:
            scs_2['passenger_groupbooking_ids'] = [(6, 0, scs_2['passenger_groupbooking_ids'])]
