...
class ConfirmOrderLabPintarWizard(models.TransientModel):
    _name = "confirm.order.labpintar.wizard"
    _description = 'Confirm Order Lab Pintar Wizard'

    booking_id = fields.Many2one('tt.reservation.labpintar','Booking',readonly=True)
    analyst_ids = fields.Many2many('tt.analyst.labpintar', 'tt_reservation_labpintar_analyst_confirm_order_wizard_rel', 'wizard_id',
    ...
    def confirm_order(self):
        if not ({self.env.ref('tt_base.group_external_vendor_labpintar_level_2').id, self.env.ref('tt_base.group_reservation_level_4').id}.intersection(set(self.env.user.groups_id.ids))):
            raise UserError('Error: Insufficient permission. Please contact your system administrator if you believe this is a mistake. Code: 168')
